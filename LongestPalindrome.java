 ...
 * date:2014/9/29
 ...
 三种做法：
 1. o(n^3), 遍历所有可能的起点和终点。brute force
 2. o(n^2) space o(n^2) 用dp, f[i][i]= true; f[i][i+1] =true if s[i]=s[i+1]; 然后再两重循环扩展之。
 3. o(n^2) 以中心扩展。 try(s, i,i) and try(s,i,i+1)   space(o1) 试一下可以扩展多少。
 4. o(n^2) f[i] 表示 以i为终点，最长是多长； 记录maxlen ,后面只要试  try(i-maxLen, i), if true, maxlen++.
 */
